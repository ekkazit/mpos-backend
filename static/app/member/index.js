...
app.factory('appFactory', function ($http) {
  ...
  factory.remove = function (id) {
    return $http.get('/api/member/delete/' + id);
  };

  factory.deleteAll = function (items) {
    return $http({
      url: '/api/member/deleteall',
      method: 'post',
      data: {
        'items': items
      }
...
app.controller('ctrl', function ($scope, $http, appFactory) {
  ...
  $scope.view = function (id) {
    window.location.href = '/admin/member/form/' + id;
  };

  $scope.remove = function (id) {
    if (confirm('คุณต้องการจะลบรายการสมาชิกใช่หรือไม่ ?')) {
      appFactory.remove(id).success(function (data) {
        if (data.result) {
          alert('ลบรายการสมาชิกสำเร็จแล้ว');
          $scope.search();
        }
      });
    }
  ...
  $scope.isSelectedAll = function () {
    return $scope.selected.length === $scope.members.length && $scope.members.length > 0;
  };

  $scope.deleteAll = function () {
    if (!$scope.selected.length)
      return;
    if (confirm('คุณต้องการจะลบรายการสมาชิกที่เลือกใช่หรือไม่ ?')) {
