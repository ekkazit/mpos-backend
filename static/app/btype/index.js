...
app.controller('ctrl', function ($scope, $http, appFactory) {
  ...
  $scope.load = function () {
    ...
    appFactory.load(isActive, term, $scope.sortOrder, $scope.sortDesc, $scope.page, $scope.rp).then(function (out) {
      var res = out.data;
      $scope.branch_types = res.branch_types;
  ...
  $scope.view = function (id) {
    window.location.href = '/admin/btype/form/' + id;
  };

  $scope.viewproduct = function (id) {
    window.location.href = '/admin/btype/product/' + id;
  };

  $scope.viewsub = function (id) {
    window.location.href = '/admin/btype/subcat/' + id;
  };

  $scope.remove = function (id) {
    if (confirm('คุณต้องการจะลบกลุ่มของสาขาใช่หรือไม่ ?')) {
      appFactory.remove(id).success(function (data) {
        if (data.result) {
          alert('ลบรายการกลุ่มสาขาสำเร็จแล้ว');
          $scope.search();
        }
      });
    }
  ...
  $scope.selectAll = function ($event) {
    var checkbox = $event.target;
    var action = (checkbox.checked ? 'add' : 'remove');
    for (var i = 0; i < $scope.branch_types.length; i++) {
      var entity = $scope.branch_types[i];
  ...
  $scope.isSelectedAll = function () {
    return $scope.selected.length === $scope.branch_types.length && $scope.branch_types.length > 0;
  };

  $scope.deleteAll = function () {
    if (!$scope.selected.length)
      return;
    if (confirm('คุณต้องการจะลบกลุ่มของสาขาที่เลือกใช่หรือไม่ ?')) {
