...
app.controller('ctrl', function ($scope, $http, appFactory) {
  ...
  $scope.viewsub = function (id) {
    window.location.href = '/admin/category/subcat/' + id;
  };

  $scope.remove = function (id) {
    if (confirm('คุณต้องการจะลบรายการประเภทสินค้าใช่หรือไม่ ?')) {
      appFactory.remove(id).success(function (data) {
        if (data.result) {
          alert('ลบรายการประเภทสินค้าสำเร็จแล้ว');
          $scope.search();
        }
      });
    }
  ...
  $scope.isSelectedAll = function () {
    return $scope.selected.length === $scope.categories.length && $scope.categories.length > 0;
  };

  $scope.deleteAll = function () {
    if (!$scope.selected.length)
      return;
    if (confirm('คุณต้องการจะลบรายการประเภทสินค้าที่เลือกใช่หรือไม่ ?')) {
